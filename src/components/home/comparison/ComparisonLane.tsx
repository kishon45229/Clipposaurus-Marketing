...
export const ComparisonLane = React.memo(({ lane }: ComparisonLaneProps) => {
    ...
    return (
        <div
            className={`group relative h-full rounded-3xl border p-4 lg:p-6 transition-all duration-300 
                ${isPreferred
        ...
        >
            <div className="flex items-start justify-between gap-2 sm:gap-4">
                <div className="space-y-2">
                    ...
                    <h3 className={`text-2xl sm:text-3xl w-full font-semibold leading-tight 
                            ${isPreferred
                ...
                <span className={`inline-flex items-center gap-2 rounded-full border p-2 text-xs font-semibold 
                        ${isPreferred
            ...
            <div className="mt-6 space-y-4">
                {lane.comparisonPoints.map((point, index) => (
                    <div key={index} className="flex gap-4">
                        <div
                            className={`mt-0.5 flex h-8 w-8 sm:h-10 sm:w-10 flex-shrink-0 items-center justify-center rounded-xl border text-sm font-semibold transition-transform duration-300 
                                ${isPreferred
                                    ? "border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-100"
                                    : "border-zinc-200 bg-zinc-100 text-zinc-800 dark:border-zinc-700 dark:bg-zinc-900"}
            ...
            <div className={`pointer-events-none absolute inset-x-0 bottom-0 h-24 rounded-b-3xl bg-gradient-to-t 
                    ${isPreferred
